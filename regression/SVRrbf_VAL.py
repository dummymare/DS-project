...
parser.add_argument('--regularization', type=float, dest='regular', default=100)
parser.add_argument('--gamma', type=float, dest='gamma', default=0.1)
parser.add_argument('--epsilon', type=float, dest='epsilon', default=0.1)
...
for train_index, test_index in kf.split(x):
    svr_rbf = SVR(kernel='rbf', C=args.regular, gamma=args.gamma, epsilon=args.epsilon)
    model = svr_rbf.fit(x[train_index], y[train_index])
